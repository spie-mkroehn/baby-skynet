...
if not exist "data\containers" mkdir "data\containers"
if not exist "data\containers\postgres" mkdir "data\containers\postgres"
if not exist "data\containers\chromadb" mkdir "data\containers\chromadb"
if not exist "data\containers\neo4j" mkdir "data\containers\neo4j"
if not exist "data\containers\neo4j-logs" mkdir "data\containers\neo4j-logs"
...
podman run -d ^
    ...
    -p 5432:5432 ^
    -e POSTGRES_DB=baby_skynet ^
    -e POSTGRES_USER=claude ^
    -e POSTGRES_PASSWORD=skynet2025 ^
...
podman run -d ^
    ...
    -e CHROMA_PORT=8000 ^
    -e ALLOW_RESET=TRUE ^
    -v "%cd%\data\containers\chromadb:/chroma/chroma" ^
...
echo    * PostgreSQL:  localhost:5432 (user: claude, password: skynet2025, db: baby_skynet)
